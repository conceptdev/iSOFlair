...
namespace iSOFlair
{
	...
	public partial class SOViewController : UIViewController
	{
		...
		private string ImagePath {
			get { return Path.Combine (DocumentsDirectory, Site.PreferencesPrefix + "gravatar.png"); }
		}
		private string CachePath {
			get { return Path.Combine (DocumentsDirectory, Site.PreferencesPrefix + "cache.txt"); }
		}
		/// <summary>
		...
		public void WireUp ()
		{
			...
			buttonSafari.TouchDown += delegate { UIApplication.SharedApplication.OpenUrl (new NSUrl (String.Format (Site.ProfileUrl, Site.SiteId))); };
		}
		...
		public bool LoadFromCache ()
		{
			...
			try {
				...
				if (cache == "") {
					...
					textDebug.Text = "";
					// blank out, nothing loaded from the web
					if (File.Exists (ImagePath)) {
						UIImage img = UIImage.FromFileUncached (ImagePath);
						// TODO: WHY didn't FromFile work, when FromFileUncached does?
						if (img != null)
		...
		public void Update ()
		{
			...
			try {
				...
				textDebug.Text = Environment.NewLine + "DOWNLOADDATA:" + Environment.NewLine + ex.Message;
			...
			try {
				...
				try {
					...
					textDebug.Text += Environment.NewLine + "DOWNLOAD IMAGE:" + Environment.NewLine + gravatarUrl + Environment.NewLine + ImagePath + Environment.NewLine + ex1.Message;
				}
				...
				try {
					if (File.Exists (ImagePath)) {
						...
						textDebug.Text += Environment.NewLine + "UIIMAGE NOT EXIST:" + Environment.NewLine + ImagePath;
					}
				} catch (Exception ex2) {
					textDebug.Text += Environment.NewLine + "FROM FILE: " + Environment.NewLine + ImagePath + Environment.NewLine + ex2.Message;
				}
				...
				string cache = String.Format ("{0}?{1}?{2}?{3}?{4}?{5}?{6}", username, points, goldBadges, silverBadges, bronzeBadges, profileUrl, dateUpdated);
				System.IO.File.WriteAllText (CachePath, cache);
				labelConnectionError.Text = "";
				// blank out if we get to here without Exception
			} catch (Exception ex) {
				labelLastUpdated.Text = ex.Message;
				textDebug.Text += Environment.NewLine + ex.Message;
			}
		}
