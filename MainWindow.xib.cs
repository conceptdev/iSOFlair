...
namespace iSOFlair
{
	...
	public partial class AppDelegate : UIApplicationDelegate
	{
		...
		int siteCount = 0;
		// TODO: remove 'hardcoded' fixed number of four sites
		int maxPageVisited = 0;
		// used to set the 'badge' number: number of un-viewed panels
		/// <summary>
		...
		public override bool FinishedLaunching (UIApplication app, NSDictionary options)
		{
			...
			documentsDirectory = documentsDirectory.Replace ("Documents", "tmp");
			// use tmp dir, don't need iTunes to back-up
			...
			pageControl.ValueChanged += delegate(object sender, EventArgs e)
			{
				...
				list[toPage].ViewController.BecomeFirstResponder ();
				// so it can "accept" shakes
		...
		private void ScrollViewScrolled (object sender, EventArgs e)
		{
			...
			if (pageNumber >= 0 && pageNumber < list.Count) {
				...
				list[(int)page].ViewController.BecomeFirstResponder ();
				// so it can "accept" shakes
				// so we can change the 'badge' on the application icon
